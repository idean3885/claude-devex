...
**무엇이 후보인가.** 사용자 교정만이 아니다. **규칙에 없어서 재량으로 정한 자리도 후보다.** 훅은 교정 신호를 정규식으로 잡으므로 재량 판정을 적립하지 못한다. 한 작업에서 선언으로 내려갈 판정 셋이 나왔는데 하나도 적립되지 않고 사람이 손으로 옮겨서 남은 사례가 있다. 옮기지 않았으면 사라졌다.

**언제 하는가.** 세션을 마치기 전이다. 훅이 턴마다 교정 신호를 후보로 적립하고, 마칠 때 `/learn` 이 세션 전체를 자산과 대조해 신규·재발·무효화를 판정한다. 자동으로 등재되는 경로는 없다. 사람이 승인한 것만 자산이 되고, 그 이유는 [승격 게이트](#승격-게이트) 에 있다.
...
수집을 작업 완료 단계에 두면 두 곳에서 새어 나간다. 완료에 도달하지 못한 작업은 통째로 빠지고, 진행 중 기록을 에이전트 자발성에 맡기면 놓친 것이 기록되지 않는다. **놓친 것이 레슨런 대상이므로 그 구조는 성립하지 않는다.** 그래서 검출은 턴마다 훅이 하고, 판단은 승격 시점 한 곳에 모은다.
...
후보 적립은 반대다. 오탐이 하나 늘어도 승격 단계에서 걸러지고 작업을 막지 않는다. 반면 미탐은 레슨런 자체를 잃는다. 그래서 신호는 넓게 잡고 판정을 뒤로 미룬다.
...
| 질문형 교정 | "이게 맞다고 봅니까?" | 일반 질문과 구분되지 않는다. 오판을 짚는 발화가 이 형태로 자주 나오지만 패턴으로 가르지 못한다 |
...
**작업을 서로 구분하는 값이어야 한다.** 트래커가 없다고 `없음` 같은 공통 값을 적으면 항목끼리 구분되지 않아 사이의 작업 수가 세지지 않는다. 값은 있는데 계산은 안 되는 상태가 되고, 그건 비워 둔 것과 결과가 같다. 세션 식별자는 후보 파일과 원본 포인터에 이미 있으므로 새로 만들 필요가 없다.
...
| 지표 | 산출 | 어디서 | 언제 |
|------|------|--------|------|
| 검출률 | 훅 후보 중 실제 교정인 수 ÷ 세션 대조로 판정된 **교정 수** | 승격 판정 결과 | 판정 직후 |
| 자산화 후 다음 재발까지 걸린 작업 수 | 자산의 `발생 이력` 두 항목 사이에 완료된 작업 수 | 자산 파일 | 재발 판정 시 |

**두 산출 모두 판정 시점에 적지 않으면 값이 복구되지 않는다.** 분모는 세션 대조가 끝나면 사라지고, 작업 수는 발생 이력의 작업 식별자로만 센다. 산출식만 두고 입력 필드를 비워 두면 지표는 정의된 채로 영구히 미측정으로 남는다.

검출률의 단위는 **발화가 아니라 교정**이다. 한 발화에 교정이 둘 담기는 경우가 실제로 나오므로, 발화로 세면 같은 세션이 세는 방식에 따라 다른 값을 낸다. 분자도 적립 건수가 아니라 **그중 실제 교정인 수**다. 훅이 하네스 알림을 발화로 잡은 적이 있고, 적립 수를 그대로 쓰면 분자가 부풀어 검출률이 실제보다 높게 읽힌다.
