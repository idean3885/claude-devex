...
스크립트도 살아 있는 마커를 덮어쓸 때 이전 갈래를 알립니다.

#### 다른 명령과 한 블록에 두지 않는다

게이트 대상 행위 **앞에** 다른 명령이 같은 블록에 있으면, 갈래가 열려 있어도 차단합니다.
...
게이트는 발동할 때만 존재가 드러납니다. 조용히 무력화되면 아무 신호가 없으므로, `scripts/pre-tool-use.mjs` 를 고쳤으면 점검을 돌립니다.

```bash
node scripts/selftest-action-gate.mjs
```

통과해야 하는 8건과 차단되어야 하는 5건을 모두 봅니다. 갈래를 전부 열어(`OPS_AGENT_ACTION_GATE_ALLOW=1`) 돌립니다. 동거 검사는 창이 열린 상태에서만 발동하고, 그 상태가 사고가 난 조건입니다.
