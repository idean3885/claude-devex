...
14 지표 밖이지만 게이트에 직접 사용되는 네 카운터. 모두 `tells_count.py` 에 구현한다.
...
- 용도: `before >= 5 AND after == 0`(윤문이 수사 구조를 몰살) 판정에만 사용한다. 문자 diff 가 못 보는 구조 편집을 잡는 보조 신호다.
...
- 대상 문자: `—`(em dash), `―`(horizontal bar). `–`(en dash)는 수치 범위 표기에 정당하게 사용되어 제외한다.
