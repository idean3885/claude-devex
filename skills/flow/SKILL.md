...
- **GATE 3 (한시 권한 개방)**: 되돌리기 어려운 행위(PR 머지·릴리즈·force push·리소스 삭제 등)는 `scripts/pre-tool-use.mjs` 가 막는다. 어시스턴트가 권한을 스스로 열지 않는다. 판정 주체는 항상 사람이다.
  ...
  1. 어시스턴트는 멈추고, 어떤 행위를 왜 하는지 플랜으로 제시한다 (자율 진행 범위 명시)
  2. 사용자가 플랜을 검토·명시 승인한다
  3. 사용자가 직접 실행한다. 어시스턴트가 감지된 갈래를 담은 명령을 제시한다: `! bash ~/.claude/ops-agent/current/scripts/action-gate-allow.sh on <갈래>`
  4. 어시스턴트는 그 창 동안 그 갈래만 수행한다. 다른 갈래는 다시 차단된다 (자율이 아니라 집행이다)
  5. 마무리 시 사용자가 `off` (선택)

  - **판정 근거는 마커의 갈래와 만료뿐이다.** 「승인」·「게이트 오픈」 같은 사용자의 문장은 2단계를 통과시키고 3단계를 예고할 뿐, 권한을 열지 않는다. 2와 3은 합쳐지지 않는다.
  - 어시스턴트가 개방·해제를 직접 실행하면 오토모드가 self-modification 으로 차단한다. 정상 동작이며 우회하지 않는다. `status` 는 읽기만 하므로 차단되지 않는다.
  - 이 절차는 무엇을 차단하는가를 바꾸지 않는다. 사람이 승인하는 경로를 기술할 뿐이다.
  - **권한 대상 행위는 단독 명령으로 실행한다.** 앞에 다른 명령을 붙이면 그 출력을 읽기 전에 행위가 실행된다. 갈래가 열려 있어도 훅이 차단한다 (예외: 판정을 종료 코드로 돌려주는 `scripts/pre-merge-check.sh`).

머지는 사용자가 웹에서 직접 수행한다 (세이프티가드). 로컬 `gh pr merge` 가 필요하면 GATE 3 을 따른다.

- **GATE 4 (이슈 생성 승인)**: `/issue create` 에서 제목·개수·상하위 구조를 제시하고 승인 대기. 이슈 트래커는 삭제 권한이 없는 경우가 많아 잘못 만든 이슈의 정리가 사용자 작업으로 넘어간다. 커밋보다 되돌리기 비용이 크다
- **GATE 5 (제출 전 변경분 자체 검증)**: PR 본문 작성 전에 변경분을 펼쳐 읽는다. 빌드 성공은 이 확인을 대신하지 못한다. 판정하는 명령은 단독으로 돌린다 ([guides/pr.md](guides/pr.md) 의 「제출 전 변경분 자체 검증」)
- **GATE 6 (머지 전 버전 대조)**: 버전 파일을 두는 레포는 `scripts/pre-merge-check.sh` 로 원격 브랜치와 타겟의 버전을 비교하고 CHANGELOG 버전 헤더 중복을 본다. 검출 시 종료 코드 1 이라 `&& gh pr merge` 로 물려도 머지가 실행되지 않는다 ([guides/pr.md](guides/pr.md) 의 「머지 전 버전 대조」)
