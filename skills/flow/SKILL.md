...
- **GATE 5 (제출 전 변경분 자체 검증)**: PR 본문 작성 전에 변경분을 펼쳐 읽는다. 빌드 성공은 이 확인을 대신하지 못한다 ([guides/pr.md](guides/pr.md) 의 「제출 전 변경분 자체 검증」)
- **GATE 6 (머지 전 버전 대조)**: 버전 파일을 두는 레포는 원격 브랜치와 타겟의 버전을 비교하고 CHANGELOG 버전 헤더 중복을 본다 ([guides/pr.md](guides/pr.md) 의 「머지 전 버전 대조」)
