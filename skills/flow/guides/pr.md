...
7. provider의 PR 생성 API/명령 실행
8. PR URL 사용자에게 전달
9. **PR 머지** (사용자가 머지를 요청한 경우):
...
리뷰어의 시간을 쓰기 전에 스스로 변경분을 한 번 읽는 것이 제출 절차의 일부다. 특정 레포의 설계 규약이 아니라 제출이라는 행위에 붙는다. 레포별 스킬에 두면 규약이 적힌 레포에만 걸리고, 정작 규약이 없는 레포에서 같은 실수가 난다.
