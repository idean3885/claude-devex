...
이미 등재된 교훈이 판단 시점에 읽히게 하는 절차다. 용어·파일 규약·재발 분석은 [../../../docs/lessons.md](../../../docs/lessons.md) 가 정본이고, 여기서는 **언제 읽고 무엇을 확인하는가**를 정한다. 아래 워크플로우가 이 절을 단계로 부른다.
...
1. 교훈 버킷 경로를 확인한다. direction profile 의 `para.root` + `para.buckets.areas`, 또는 소비 프로젝트의 규칙 문서가 선언한 경로
2. 인덱스를 확보한다. 버킷에 인덱스 파일이 있으면 그것을 읽고, 없으면 버킷 안 교훈 파일의 제목·상황 줄만 모아 인덱스로 쓴다. 이 단계에서 본문은 열지 않는다
3. 결과를 아래 네 문구 중 하나로 표시한다. 표시한 뒤에 다음 단계로 간다

| 결과 | 표시 문구 | 다음 |
|------|-----------|------|
| 인덱스 있음 | `교훈 인덱스: N건 로드 (<버킷 경로>)` | 대조 지점에서 쓴다 |
| 버킷은 정의됐고 항목 0건 | `교훈 인덱스: 0건 (<버킷 경로>)` | 그대로 진행 |
| 버킷 미정의 | `교훈 인덱스: 버킷 미정의` | 그대로 진행. 새 위치를 만들지 않고, 어디에 둘지 사용자에게 물어 둔다 |
| 버킷은 정의됐고 인덱스를 읽지 못함 | `교훈 인덱스: 로드 실패 <경로> (<사유>)` | 그대로 진행. 대조 지점에서 이 문구를 다시 적는다 |
...
인덱스 전 항목을 훑어 지금 판단에 걸리는 항목을 고르고, 걸린 항목의 본문을 열어 원천까지 확인한다. 확인 결과를 `대조 N건 / 걸림 M건` 형식으로 적는다. 걸림이 없으면 `걸림 0건` 이라고 적는다.
...
**워크플로우**:
1. **교훈 인덱스 로드**: 위 [교훈 인덱스](#교훈-인덱스) 의 로드 절차를 수행하고 결과 문구를 표시
2. provider 정의 파일 읽기
3. 이슈 제목 및 내용 파악 (사용자에게 질문)
4. **작업 단위 결정 + 상위-하위 관계 체크**:
   - 한 용건을 이슈 몇 건으로 둘지 정하기 전에 [교훈 인덱스 대조](#대조-판단-지점마다) 를 수행하고 `대조 N건 / 걸림 M건` 을 적는다
...
5. provider 정의에 따라 라벨/태그 매핑
6. 이슈 본문 작성 (provider의 본문 템플릿 사용)
7. **대외비 가드 (GATE 0)**: 제목·본문 전체에 대해 `references/confidential-guard.md` 검증. 히트 시 사용자 정정 후 재검증
8. provider의 이슈 생성 API/명령 실행 (provider에 정의된 인증·endpoint·body 그대로 사용)
9. **Extensions 실행**: provider에 post-create Extensions이 정의되어 있으면 실행
...
**워크플로우**:
1. **교훈 인덱스 로드**: 위 [교훈 인덱스](#교훈-인덱스) 의 로드 절차를 수행하고 결과 문구를 표시. 이 표시 없이 2번으로 넘어가지 않는다
2. provider 정의 파일 읽기
...
9. **코드 작업 여부 확인**: "코드 작업이 필요한 이슈입니까?"
   - **Yes**: 브랜치명·베이스를 확정하기 전에 [교훈 인덱스 대조](#대조-판단-지점마다) 를 수행하고 `대조 N건 / 걸림 M건` 을 적는다. 그다음 provider의 브랜치 패턴과 리모트 머지 이력에 남은 접두를 대조해 브랜치명을 정하고, `origin/{base}` 에서 분기한다
...
11. **교훈 수집**: 이 이슈 기간에 기록된 교정 항목을 수집한다 ([../../../docs/lessons.md](../../../docs/lessons.md))
    ...
    - 시작 시 로드한 인덱스에 걸렸는데도 같은 오판이 났으면 재발로 잡고, 원인을 `docs/lessons.md` 의 원인 목록에서 고른다
    - 수집 구현·버킷 경로는 소비 프로젝트가 정한다. 버킷이 미정의면 `교훈 버킷 미정의` 로 표시하고 통과한다. 새 위치를 만들지 않는다
...
- 교훈 인덱스 로드는 결과 문구를 남긴다. 로드된 건수, 0건, 버킷 미정의, 로드 실패를 각각 다른 문구로 적는다
